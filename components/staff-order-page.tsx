...
export default function StaffOrderPage({ onNavigate, currentUser, onLogout }: StaffOrderPageProps) {
  ...
  const getFilteredItems = () => {
    let items = menuData.flatMap(category => 
      category.products.filter(item => item.available).map(item => ({
        ...item,
      ...
      <div className="bg-emerald-700 text-white p-4 sm:p-6">
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-xl sm:text-2xl font-bold">Staff Assisted Order</h1>
              <p className="text-emerald-100 text-sm">Help customers place their orders</p>
            </div>
          </div>
          <div className="flex items-center gap-4">
            <div className="flex items-center gap-2 text-emerald-100">
              <User className="w-4 h-4" />
              <span className="text-sm">{currentUser?.name || 'Staff User'}</span>
            </div>
            <Button
              variant="ghost"
              size="sm"
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-4">
          ...
          <div>
            <Label htmlFor="staff" className="text-white">Staff Member *</Label>
          ...
          <div>
            <Label htmlFor="customer" className="text-white">Customer Name</Label>
            <Input
              id="customer"
              value={customerName}
      ...
      <div className="flex flex-col">
        ...
        <div className="lg:hidden bg-white border-b border-gray-200 p-4 sticky top-0 z-10">
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-4">
              <span className="font-bold text-emerald-600">₹{getTotalAmount().toFixed(2)}</span>
              <Button
                size="sm"
                onClick={placeOrder}
                className="bg-emerald-600 hover:bg-emerald-700"
                disabled={!selectedTable || !selectedStaff || cartItems.length === 0}
              >
                Place Order
              </Button>
            </div>
          ...
          <div className="flex-1 p-4 sm:p-6 lg:pr-0">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-2 xl:grid-cols-3 gap-4 pb-20 lg:pb-4">
              {getFilteredItems().map((item) => {
                ...
                const priceDisplay = item.rate === 'APS' ? 'APS' : `₹${price}`
                
                return (
                  <Card key={item.itemNo} className="hover:shadow-md transition-shadow">
                    <CardContent className="p-4">
                      <div className="flex justify-between items-start mb-2">
                        <div className="flex-1">
                          <h4 className="font-medium text-gray-900 text-sm mb-1">{item.name}</h4>
                          <p className="text-xs text-gray-500">#{item.itemNo} • {item.categoryName}</p>
                        </div>
                      ...
                      <div className="flex justify-between items-center">
                        ...
                        <Button
                          size="sm"
                          onClick={() => addToCart(item.itemNo, item.name, item.rate)}
                  ...
                  <>
                    ...
                    <div className="border-t p-6">
                      <div className="flex justify-between items-center mb-4">
                        ...
                        <span className="font-bold text-xl text-emerald-600">₹{getTotalAmount().toFixed(2)}</span>
                      </div>
